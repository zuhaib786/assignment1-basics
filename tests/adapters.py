...
def run_swiglu(
    ...
    Args:
        d_model: Dimensionality of the feedforward input and output.
        d_ff: Dimensionality of the up-project happening internally to your swiglu.
        w1_weight: Stored weights for W1
        w2_weight: Stored weights for W2
        w3_weight: Stored weights for W3
        in_features: Input embeddings to the feed-forward layer.

    Returns:
        Output embeddings of the same shape as the input embeddings.
    """
    # Example:
...
def run_multihead_self_attention(
    ...
    Args:
        ...
        q_proj_weight: Weights for the Q projection
        k_proj_weight: Weights for the K projection
        v_proj_weight: Weights for the V projection
        o_proj_weight: Weights for the output projection
        in_features: Tensor to run your implementation on.
...
def run_multihead_self_attention_with_rope(
    ...
    Args:
        ...
        o_proj_weight: Weights for the output projection
        in_features: Tensor to run your implementation on.
        token_positions: Tensor of shape (batch_size, sequence_length) with the token positions
...
def run_rope(
    ...
    Args:
        ...
        max_seq_len: Maximum sequence length to pre-cache if your implementation does that.
        in_query_or_key: Input tensor to run RoPE on. Shape is (batch_size, sequence_length, d_k).
        token_positions: Tensor of shape (batch_size, sequence_length) with the token positions
    Returns:
        Tensor with of the RoPE encoded input.
    """
    raise NotImplementedError
...
def run_rmsnorm(
    ...
    Args:
        d_model: The dimensionality of the RMSNorm input.
        eps: A value added to the denominator for numerical stability.
        weights: RMSNorm weights.
            in_features: Input features to run RMSNorm on. Can have arbitrary leading
            dimensions.
...
def run_silu(in_features: torch.FloatTensor) -> torch.FloatTensor:
    """Given a tensor of inputs, return the output of applying SiLU
    to each element.

    Args:
        in_features: torch.FloatTensor
            Input features to run SiLU on. Shape is arbitrary.
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    Args:
        ...
        dim: Dimension of the `in_features` to apply softmax to.

    Returns:
        Tensor of with the same shape as `in_features` with the output of
        softmax normalizing the specified `dim`.
    """
    raise NotImplementedError


def run_cross_entropy(inputs: Float[Tensor, " batch_size vocab_size"], targets: Int[Tensor, " batch_size"]):
